...
paid as (
    select
        ...
        sum(case when is_paid_post_call then 1 else 0 end) as paid_post_call_count,
        ...
        (sum(case when coalesce(used_estimated_fx_rate, false) then 1 else 0 end) > 0) as value_recovered_usd_is_estimated,
        sum(case when not is_window_closed then 1 else 0 end) as still_in_window_count
...
select
    ...
    {{ dbt_utils.safe_divide('coding_paid.coded_calls', 'coding_paid.total_outbound_calls') }} as coding_rate,
    coding_paid.total_dispositions,
    coding_paid.paid_post_call_count,
    {{ dbt_utils.safe_divide('coding_paid.paid_post_call_count', 'coding_paid.total_dispositions') }} as paid_post_call_rate,
